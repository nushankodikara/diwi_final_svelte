<script lang="ts">
	...
	const saveNote = (auth: any, user: any) => {
		addDoc(collection(db, user.uid, 'status', 'notes'), {
			note: note,
...
<SignedIn let:auth let:user>
	<!-- Notes -->
	<Collection ref="{user.uid}/status/notes" let:data let:count>
		<p class="p-2 text-sm font-thin text-gray-400 dark:text-white">
			You have {count} Status Records
		</p>
		<div
			...
			{#each orderData(data) as post}
				{#if post.uid === user.uid}
					<div class="flex w-full items-start gap-2.5">
						...
						<img class="h-8 w-8 rounded-full" src={svg} alt="Jese image" />
						<div
	...
	<div class="fixed bottom-0 left-0 right-0 mb-16 flex flex-row gap-4 bg-white p-4">
		<Input type="email" placeholder="Status Note" size="md" bind:value={note} />
		<Button
			on:click={() => {
				saveNote(auth, user);
			}}>Save</Button

<script lang="ts">
	...
	const sendMessage = async (user: any) => {
		...
		await addDoc(collection(db, `/counselor/${user.uid}/chats`), {
			text: message,
			createdAt: serverTimestamp(),
			chatuid: data.props.id,
			uid: user.uid
		});
		message = '';
	};
...
<SignedIn let:user>
	<Collection ref={query(collection(db, `/counselor/${user.uid}/chats`), where('chatuid','==',data.props.id), orderBy('createdAt', 'asc'), limit(40))} let:data let:count>
